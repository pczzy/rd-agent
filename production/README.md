...
- **目标组合** —— 全池 300 只按模型打分从高到低排，**未入选的逐条给出理由**
  （一手超上限 / 流动性未达标 / 名额已满 / 资金不足 / 无报价），入选行整行淡染。
  显示范围可切换「入选 30 只 / 前 60 名 / 全池 300 只」，技术面按需附加。

  这一页要回答的是"为什么模型最看好的没买"。09-03 那次，打分前 5 里有 4 只是
  **一手超上限** —— 海光信息一手 24,620 元、寒武纪 109,999 元，而单只上限只有 9,000 元。
  它们是被账户规模挡住的，不是模型不看好；把这个原因显示出来，才不会误以为信号选得差。
...
| 项目 | 标准 | 沪市 | 深市 | 出处 |
|---|---|---|---|---|
| 佣金 | `max(5 元, 成交额 × 0.02354%)` | 收 | 收 | **成交回单实测**（见下），上限见国金《佣金标准》3‰、起点 5 元 |
| 过户费 | 成交额 × 0.001%（双向） | **单独收** | 已含在佣金里 | 中国结算：A 股 0.01‰ 双向 |
| 印花税 | 成交额 × 0.05% | 仅卖出 | 仅卖出 | 中国结算 1‰ 减半征收；**回单实测** 17.50/35,000 = 0.05% |
...
**费率来自回单实测，不是费率表。** 五张成交回单逐项复现（`fee_parts()` 的输出与回单
的佣金/过户费/印花税三行分别相等，不只是合计相等）：

| 回单 | 成交额 | 佣金 | 过户费 | 印花税 |
|---|---|---|---|---|
| 09-04 买 阳光电源（深） | 8,880 | 5.00 | **0.00** | 0.00 |
| 09-07 买 兆易创新（沪） | 38,245 | 9.00 | 0.38 | 0.00 |
| 09-07 买 兆易创新（沪） | 38,255 | 9.01 | 0.38 | 0.00 |
| 09-10 卖 兆易创新（沪） | 77,400 | 18.22 | 0.77 | 38.70 |
| 08-27 卖 山东黄金（沪） | 35,000 | 8.24 | 0.35 | 17.50 |

覆盖三只股票、四个日期、沪深两市、买卖双向，并且各自验证了一件事：

- **佣金万 2.354** —— 四笔未触发下限的单子用同一费率全部还原到分（26.25+18.22 与各自成交额）。
  费率表只给上限 3‰，实际费率一户一议，问不出来，只有回单能。
- **5 元下限确实存在** —— 深市那笔 8,880 × 万2.354 = 2.09 元，实收 5.00。
- **深市不另收过户费** —— 那笔回单的过户费一栏是 **0.00**，坐实了费率表"深市佣金包含
  过户费"的说法；此前这只是费率表的字面表述，没有实证。
- **印花税 0.05% 且仅卖出** —— 卖单 17.50/35,000 与 38.70/77,400 都是 0.05%，买单为 0.00。
- **过户费 0.001% 且仅沪市** —— 0.35/35,000、0.77/77,400、0.38/38,255 都是 0.001%。回单只列佣金/过户费/印花税三行、没有单列规费，
所以经手费 0.0341‰ 和证管费 0.02‰ 应当已并在佣金里。此前配置写的 0.075% 是凭印象填的，
**高了 3.2 倍**，导致每笔费用都算高约三倍。费率表只给上限（3‰），问不出实际费率，
只有回单能。

由此 `cost_one_side` 的推导也要跟着变：原式是 佣金 0.075% + 过户 0.001% + 半价差 0.030%
+ 冲击 0.085% = 0.191%；换成实测佣金后是 **0.02354% + 0.001% + 0.030% + 0.085% ≈ 0.140%**。
这个数进回测会显著改变结果，所以**留待用生产口径重跑时一并更新**（见 WORKLOG 的 TODO），
现在没动。
