...
class QlibCondaConf(CondaConf):
    ...
    # qlib's `qrun` records experiments into a local ./mlruns file store, which mlflow>=3
...
class QlibCondaEnv(LocalEnv[QlibCondaConf]):
    QLIB_COMMIT = "2fb9380b342556ddb50a4b24e4fe8655d548b2b8"

    def _env_exists(self) -> bool:
        result = subprocess.run(
            f"conda env list | grep -q '^{self.conf.conda_env_name} '",
            shell=True,
        )
        return result.returncode == 0

    def _qlib_installed(self) -> bool:
        result = subprocess.run(
            f"conda run -n {self.conf.conda_env_name} python -c 'import qlib'",
            shell=True,
            capture_output=True,
        )
        return result.returncode == 0

    def _pip_install(self, spec: str, retry_count: int = 3, retry_wait_seconds: int = 10) -> None:
        """pip install in the conda env, retrying transient network/TLS failures."""
        cmd = f"conda run -n {self.conf.conda_env_name} pip install {spec}"
        for attempt in range(1, retry_count + 1):
            if subprocess.run(cmd, shell=True).returncode == 0:
                return
            if attempt < retry_count:
                print(f"[yellow]`pip install {spec}` failed, retrying ({attempt}/{retry_count})...[/yellow]")
                time.sleep(retry_wait_seconds)
        raise RuntimeError(f"Failed to install `{spec}` into conda env '{self.conf.conda_env_name}'")
    ...
    def prepare(self) -> None:
        ...
        # The env may exist but be half-installed (e.g. an earlier run died while pip was
        # fetching qlib), so check the packages instead of trusting `conda env list`.
        if not self._qlib_installed():
            self._pip_install("--upgrade pip cython")
            self._pip_install(f"git+https://github.com/microsoft/qlib.git@{self.QLIB_COMMIT}")
            self._pip_install("catboost xgboost tables torch")

        _CONDA_ENV_PREPARED.add(self.conf.conda_env_name)

        # bin_path is resolved when the conf is built, which may be before the env existed.
...
class QlibDockerConf(DockerConf):
    ...
    default_entry: str = "qrun conf.yaml"
    # see QlibCondaConf: mlflow>=3 refuses qlib's ./mlruns file store without this.
    env_dict: dict = {"MLFLOW_ALLOW_FILE_STORE": "true"}
    extra_volumes: dict = {
...
class QTDockerEnv(DockerEnv):
    ...
    def __init__(self, conf: DockerConf | None = None):
        # NOTE: build a fresh conf per instance; a mutable default is shared across all
        # instances, so one caller editing env.conf would affect every other QTDockerEnv.
    ...
    def prepare(self, *args, **kwargs) -> None:  # type: ignore[no-untyped-def]
        ...
        """
        super().prepare()
        default_qlib_data_path = str(Path("~/.qlib/").expanduser().resolve().absolute())
        qlib_data_path = next(iter(self.conf.extra_volumes.keys()), default_qlib_data_path)
