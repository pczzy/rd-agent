...
def get_runtime_environment_by_env(env: Env) -> str:
    ...
    if json_match is None:
        # Usually means the environment is broken: `python` is missing from it, or the
        # script died before printing. Surface the output instead of an opaque TypeError.
        raise RuntimeError(
            f"Failed to collect runtime info from {type(env).__name__}: no JSON in the output below.\n"
            f"{stdout.strip()}"
        )
