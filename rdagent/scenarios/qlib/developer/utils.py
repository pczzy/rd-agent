...
def normalize_model_type(model_type: str) -> str:
    """Map a task's model_type onto the two values the qlib configs understand.

    The LLM sometimes echoes the prompt placeholder ("Tabular or TimeSeries") instead of
    picking one. Previously that matched neither branch, so `num_features` never reached
    the container: the template rendered an empty value, YAML read it as null, and the
    model blew up much later inside nn.Linear(None). Fail loudly here instead.
    """
    if not isinstance(model_type, str):
        raise ModelEmptyError(f"model_type must be a string, got {type(model_type).__name__}")
    normalized = model_type.strip().lower().replace(" ", "").replace("-", "").replace("_", "")
    if normalized == "timeseries":
        return "TimeSeries"
    if normalized == "tabular":
        return "Tabular"
    raise ModelEmptyError(f"Unsupported model_type {model_type!r}; expected exactly 'Tabular' or 'TimeSeries'.")


def feature_columns(columns) -> pd.MultiIndex:
    """Build the ('feature', <name>) column index used by combined_factors_df.parquet.

    Names must be native `str`. A numpy scalar (e.g. np.str_) survives as-is otherwise,
    and under numpy>=2 its repr is `np.str_('x')`; pandas writes the column repr into the
    parquet metadata and pyarrow reads it back with ast.literal_eval, which rejects that
    call expression with "malformed node or string". The backtest then dies at load time.
    """
    return pd.MultiIndex.from_product([["feature"], [str(c) for c in columns]])
