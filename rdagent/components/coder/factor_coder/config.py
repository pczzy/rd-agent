...
def get_factor_env(
    ...
    # Factor code runs in whichever env this process lives in. CONDA_DEFAULT_ENV is only
    # exported by an interactive `conda activate`, so it is absent under systemd/cron and
    # CondaConf then fails validation on None. Derive it from sys.prefix in that case.
    conda_env_name = os.environ.get("CONDA_DEFAULT_ENV") or Path(sys.prefix).name
